...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    getUser: function getUser() {
      ...
      axios.get('users/getUser').then(function (response) {
        _this.username = response.data.data.email;
        var access = response.data.data.roles;
        var checker = 0;
        access.forEach(function (doc) {
          if (doc.id == 2) {
            checker = 1;
          } else {
            checker = 0;
